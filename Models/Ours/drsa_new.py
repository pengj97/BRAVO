...
def ours(setting, attack, flag_time_varying, optimizer):
    """
        ...
        for id in range(conf['nodeSize']):
            ...
            if setting == 'iid':
                ...
                if id in Config.regular:
                    ...
                    count += 1
            if optimizer == 'spider' :
                conf['iter_last'][id] = workerPara_memory[id].copy ()
        workerPara_memory = workerPara.copy()
    ...
    output = open("../../experiment-results/drsa-"+optimizer+last_str+"-"+setting+"-constant.pkl", "wb")
    pickle.dump((para_list, loss_list), output, protocol=pickle.HIGHEST_PROTOCOL)
    print(optimizer)


if __name__ == '__main__':
    ours(setting='iid', attack=None, flag_time_varying=False, optimizer='svrg')